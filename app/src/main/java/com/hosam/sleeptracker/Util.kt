...
package com.hosam.sleeptracker
...
fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            if (it.ndTimMilli != it.startTimeMilli) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.ndTimMilli)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.SleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.ndTimMilli.minus(it.startTimeMilli) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.ndTimMilli.minus(it.startTimeMilli) / 1000 / 60}:")
                // Seconds
                append("${it.ndTimMilli.minus(it.startTimeMilli) / 1000}<br><br>")
            }
        }
    }
